	...
	implements ScoreLayouterStrategy
{
		...
		boolean drawCaption, boolean drawLeftHook, boolean drawRightHook)
	{
		...
		float x1 = staff.getMeasureStartMm(startMeasureIndex) + staff.getInterlineSpace() / 2;
		float x2 = staff.getMeasureEndMm(endMeasureIndex) - staff.getInterlineSpace() / 2;
		//line position of volta line: 5 IS over top line
		float lp = (staff.getLinesCount() - 1 + 5) * 2;
		//caption
		FormattedText caption = null;
		if (drawCaption && volta.getCaption().length() > 0)
		{
			caption = new FormattedText(volta.getCaption(), textStyle, Alignment.Left);
		}
		//create stamping
