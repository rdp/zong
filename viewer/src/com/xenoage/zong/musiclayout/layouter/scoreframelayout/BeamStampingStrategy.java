	...
	implements ScoreLayouterStrategy
{
		...
		Globals globals)
	{
		...
		float rightX = beamedStems.getLastStem().getPositionX();
		float leftLP = beamedStems.getFirstStem().getEndLinePosition() +
			beamedStems.getFirstStem().getDirection() * lineHeight / 2;
    ...
    {
    	float leveledLP = beamedStems.getFirstStem().getDirection() *
    	...
    	for (int iChord = 0; iChord < waypoints.size(); iChord++)
    	{
    		...
    		else if (wp == Waypoint.Stop || wp == Waypoint.StopHookRight)
    		{
    			...
    			float stopX = beamedStems.getStemX(iChord) +
    				(wp == Waypoint.StopHookRight ? hookLength * firstChordStaff.getInterlineSpace() : 0);
    		...
    		{
    			//left hook
    			float length = hookLength * firstChordStaff.getInterlineSpace();
    ...
    for (OpenBeamMiddleStem openStem : beamedStems.getMiddleStems())
    {
    	...
    	else if (beam.getVerticalSpan(globals) == VerticalSpan.TwoAdjacentStaves)
    	{
    		...
    		float leftStemEndMm = beamedStems.getFirstStem().getParentStaff().computeYMm(leftLP);
    		float rightStemEndMm = beamedStems.getLastStem().getParentStaff().computeYMm(rightLP);
