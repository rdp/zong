 ...
 */
public class MxlCompressedFileInput
{
	...
	/**
	 * Creates a {@link MxlCompressedFileInput} instance for the given
	 * compressed MusicXML data, which is extracted in a new (unique) subdirectory
	 * of the given folder.
	 */
	public MxlCompressedFileInput(InputStream inputStream, File osTempFolder)
		throws IOException
	{
		...
		try
		{
			...
			switch (type)
			{
				...
				case XMLOpus:
					rootItem = new MxlOpusFileInput().readOpusFile(bis);
					break;
		...
		throws IOException
	{
		...
		if (isOpus())
		{
			getScoreFilenames(new MxlOpusFileInput().resolveOpusLinks(
		...
		throws InvalidFormatException, IOException
	{
		...
		switch (fileType)
		{
			...
			case XMLScorePartwise:
				return new MxlScoreFileInput().read(bis, path, null);
	...
	public void close()
	{
		FileTools.deleteDirectory(tempFolder);
	}
